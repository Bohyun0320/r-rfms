...
//    keyColumnName: 'id',
    ...
    useClientSort: false,
    columns: [
      { title: '점군데이터ID', name: 'las_id', sortable:true, minWidth:100, align : 'center'},
...
//    console.dir(data);
    ...
    if (!data.responseData.result) {
      return ;
      // return runToastr('error', data.responseData.message);
    }
    
    // var contents = data.responseData.data.contents;
    var contents = preProcResultData(data.responseData.data.contents);
...
function setNavItem() {
  ...
  rows.each(function (el, index) {
    let row = $(this);
    let idContents = row.find('td[data-column-name=las_id] .tui-grid-cell-content');
...
function preProcResultData(contents) {
  if (!contents) return contents;

  for(let i=0; i<contents.length; i++) {
    if (contents[i].org_file_nm.toLowerCase().endsWith('.zip')) {
      contents[i].org_file_nm = contents[i].org_file_nm.slice(0, -4);
    }
  }
...
//  console.dir(gridData[gridEvent.rowKey]);
  ...
  // dataView.init("dataViewModal", dataView.MOD_LAS, gContentList[gridEvent.rowKey]);

  // navTo
  const navOption = {
    reqParams : gCurPageData.requestParameter,
    reqUrl : '/api/data/read/lidar',
    returnUrl : '/data/list/lidar',
    contents : gCurPageData.responseData.data.contents,
    totalCount : Number(gCurPageData.responseData.data.pagination.totalCount),
    key : 'las_id'
  }

  dataView.init("dataViewModal", dataView.MOD_LAS, gContentList[gridEvent.rowKey], navOption);
}
